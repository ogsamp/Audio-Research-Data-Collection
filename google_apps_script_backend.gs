...
function doGet(e){var cb=e&&e.parameter&&(e.parameter.callback||e.parameter.cb),a=e&&e.parameter&&e.parameter.action;try{if(a==="stats")return out_(stats_(),cb);if(a==="transcriptionStatus")return out_({status:"success",version:"V24.0",engine:"OpenAI Whisper",apiKeyConfigured:!!PropertiesService.getScriptProperties().getProperty("OPENAI_API_KEY"),autoTranscribeOnSubmit:autoTx_(),autoCodeOnTranscribe:autoCode_()},cb);if(a==="codePendingBatch")return out_(codePendingBatchObj_(),cb);return out_({status:"success",message:"Backend V24.0 running",autoTranscribeOnSubmit:autoTx_(),autoCodeOnTranscribe:autoCode_()},cb);}catch(err){return out_({status:"error",message:String(err)},cb);}}
...
/* V24.0 Populated Board-Ready Report Fix */
...
function researchReport_(){
  ...
  report.push("System Version: Audio Research Tool V24.0");
  report.push("");
...
/* ===================== V24.0 Local Whisper Transcription Mode Backend ===================== */
...
/* =================== End V24.0 Local Whisper Backend =================== */
